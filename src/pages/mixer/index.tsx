...
const RateLimitCard = ({
  ...
  const [remainingTime, setRemainingTime] = useState(new Date(time.getTime() - new Date().getTime()));
    ...
    <Card className=" w-[50%]">
      <CardHeader>
        ...
        <CardDescription className="text-xl">
          Please wait {remainingTime.getMinutes()} minutes and {remainingTime.getSeconds()} seconds to mix another song, this is due
...
const MixerPage = () => {
  ...
  const songOneImageUrl = useMemo(() => {
    if (songOne?.image) {
      return URL.createObjectURL(new Blob([songOne.image.data], { type: songOne.image.mimeType }));
    }
    return null;
  }, [songOne?.image]);
  ...
  const songTwoImageUrl = useMemo(() => {
    if (songTwo?.image) {
      return URL.createObjectURL(new Blob([songTwo.image.data], { type: songTwo.image.mimeType }));
    }
    return null;
  }, [songTwo?.image]);
  ...
  if (rateLimitTime) {
    return <div className="mt-6 justify-center items-center w-full">
        ...
        <Card className="bg-black w-[80%]">
          <CardContent>
            <div className="flex flex-row justify-center items-center space-x-4">
              ...
              >
                ...
                >
                  ...
                  >
                    <CardTitle className="text-white text-lg mb-2">Upload your song</CardTitle>
                    <CardDescription className="text-gray-200">Click to upload a file</CardDescription>
                  </div>
                </div>
                <input
                  type="file"
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    backgroundSize: "cover",
                    backgroundPosition: "center",
                    filter: cardTwoHovered && !songTwo?.image ? "blur(4px)" : "none",
                  }}
                />
                {songTwo && (
                  <div className="absolute top-0 left-0 right-0 p-3 bg-black/70 backdrop-blur-sm z-10">
                    <p className="text-white text-sm font-medium truncate">{songTwo.title}</p>
                  </div>
                )}
                <div
                  className={`absolute inset-0 flex items-center justify-center transition-all duration-300 z-10 ${
                    cardTwoHovered ? "opacity-100 scale-100" : "opacity-0 scale-95"
                  }`}
                >
                  <div
                    className="text-center p-4 rounded-lg bg-black/20 backdrop-blur-sm shadow-2xl border border-white/10"
            ...
            <div className="mt-4 text-center items-center w-full justify-center">
              <Button disabled={!songOne && !songTwo} size={"lg"} className="bg-[var(--text-branding)] hover:bg-[#ee31ee]/80 text-white w-[30%]">
